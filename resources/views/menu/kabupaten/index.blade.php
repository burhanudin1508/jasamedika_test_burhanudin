    ...
    <script>
        ...
        $('body').on('click', '#activate-wilayah', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "GET",
                        url: "{{ url('activate-kabupaten') }}" + '/' + post_id,
                        success: function(data) {
                            $("#post_id_" + post_id).remove();
                            Swal.fire({
                                icon: 'success',
                       ...
                       success:function(data)
                       {
                         if(data){
                            ...
                            $.each(data, function(key, provinsi){
                                ...
                                $('select[name="provinsi"]').append('<option value="'+ provinsi.id +'">' + provinsi.nama_daerah+ '</option>');
                                }else{
                                    if(provinsi.id == val ){
                                    $('select[name="provinsi"]').append('<option value="'+ provinsi.id +'" selected>' + provinsi.nama_daerah+ '</option>');
                                    }else{
                                    $('select[name="provinsi"]').append('<option value="'+ provinsi.id +'">' + provinsi.nama_daerah+ '</option>');
                                    }
                                }
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '#edit-wilayah', function() {
                var post_id = $(this).data('id');
                $.get('get-kabupaten/' + post_id, function(data) {
            ...
            $('body').on('click', '#delete-wilayah', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "GET",
                            url: "{{ url('delete-kabupaten') }}" + '/' + post_id,
                            success: function(data) {
                                $("#post_id_" + post_id).remove();
                                Swal.fire({
                                    icon: 'success',
                    ...
                    <div class="content-header row">
                        <div class="content-header-left col-md-9 col-12 mb-2">
                            <div class="row breadcrumbs-top">
                                <div class="col-12">
                                    <h2 class="content-header-title float-start mb-0">Data Kabupaten</h2>
                                </div>
                            </div>
                        </div>
                    </div>
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    @foreach ($data as $item)
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td>{{ $item->provinsi->nama_daerah }}</td>
                                                                            <td>
                                                                                @if ($item->status == 1)
                                                                            ...
                                                                            <td>
                                                                                <a href="{{ url('detail-view-kabupaten',\Crypt::encrypt($item->id) )}}"
                                                                                    id="detail-wilayah"
                                                                                    data-id=""
